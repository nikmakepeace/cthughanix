...
void newCthughaDisplay() {
    cthughaDisplay = new CthughaDisplayX11();
}


CthughaDisplayX11::CthughaDisplayX11() : CthughaDisplay() {
...
void CthughaDisplayX11::expandPalette(int narrow) {
	...
	for(int i=height; i != 0; i--) {
	    ...
	    for(int j=2*BUFF_WIDTH/4; j != 0; j--) {
		unsigned long a, b;
		unsigned long c = *buff; buff ++;
		a = bitmap_colors0[c & 0xff]; c >>= 8;
		b = bitmap_colors0[c & 0xff]; c >>= 8;
		*scrn = a | ((b & 0xff) << 24); scrn ++;
		a = b >> 8;
		
		b = bitmap_colors0[c & 0xff]; c >>= 8;
		*scrn = a | ((b & 0xffff) << 16); scrn ++;
		a = b >> 16;
	...
	for(int i=height; i != 0; i--) {
	    ...
	    for(int j=2*BUFF_WIDTH/4; j != 0; j--) {
		unsigned long b = *buff; buff ++;
		*scrn = bitmap_colors0[b & 0xff]; b >>= 8; scrn ++; 
		*scrn = bitmap_colors0[b & 0xff]; b >>= 8; scrn ++; 
		*scrn = bitmap_colors0[b & 0xff]; b >>= 8; scrn ++; 
		*scrn = bitmap_colors0[b]; scrn ++;
	    }
	    dst += expandedBufferWidth;
	    buffer += 2*BUFF_WIDTH;
	}
...
void CthughaDisplayX11::expandPaletteMirrorHV() {
	...
	for(int i=0; i < BUFF_HEIGHT; i ++) {
	    ...
	    unsigned long * scrn3 = (unsigned long*)(expandedBuffer + 
						     (2*BUFF_HEIGHT - i) * expandedBufferWidth);
		...
		unsigned long a = bitmap_colors0[b & 0xff]; b >>= 8;
		*scrn1 = a; scrn1 ++;
		*scrn2 = a; scrn2 --;
		*scrn3 = a; scrn3 ++;
		*scrn4 = a; scrn4 --;
