...
int cth_init(int * argc, char *argv[]) {
    ...
    DisplayDeviceX11::xcth_toplevel = XtAppInitialize(&xcth_app_con, 
				    "Cthugha", 
				    NULL, 0,
...
void DisplayDeviceX11::CreateWindow(char * name, int full_screen) {
    ...
    window = XCreateWindow(xcth_display, 
			   DefaultRootWindow(xcth_display), 
			   window_do_pos ? window_pos.x : 0, 
			   window_do_pos ? window_pos.y : 0,
			   disp_size.x, disp_size.y, 
...
void DisplayDeviceX11::prePrint() {
	...
	if(panelTextWidget) {
	    ...
	    XFillRectangle(xcth_display, textPixmap, gc, 
			   0,0, text_size.x * fontSize.x, text_size.y * fontSize.y);
	} else {
	    switch(shmLevel) {
	    case shmPixmap:
		break;
	    case shmImage:
		XShmPutImage(xcth_display, textPixmap, gc, image,0,0,
			     0, 0, disp_size.x, disp_size.y, 
			     0);
		break;
	    case shmNone:
		XPutImage(xcth_display, textPixmap, gc, image, 0,0,
			  0, 0,  disp_size.x, disp_size.y);
	    }
	}
    }
...
void DisplayDeviceX11::printString(int x, int y, const char * text, int color, int len, 
				   int noDarken) {

    XSetForeground(xcth_display, gc, textColor[color]);
    
    if(textPixmap) {
	XDrawString(xcth_display, textPixmap, gc, 
		    x, y + fontSize.y, 
		    text, len);
	
	copyText = 2;
    } else
	XDrawString(xcth_display, pixmap, gc, 
		    x, y + fontSize.y, 
		    text, len);
...
void DisplayDeviceX11::allocImage() {
	...
	if( (image = XShmCreateImage(xcth_display, visual, planes,
	...
	if ( (shminfo.shmid = shmget(IPC_PRIVATE, bytes_per_line * disp_size.y,
				     IPC_CREAT|0777)) == -1 ) {
	...
	if( (pixmap = XShmCreatePixmap(xcth_display, window, image->data, &shminfo, 
				       disp_size.x, disp_size.y, planes)) == 0) {
	...
	if( (image = XCreateImage(xcth_display, visual, planes, 
				  ZPixmap, 0, NULL, 
				  disp_size.x, disp_size.y,
				  XBitmapPad(xcth_display),  
				  0 /*bytes_per_line will be computed */)) == NULL) {
	    printfe("Can not create XImage.\n");
	    exit(0);
	}
	bypp = (image->bits_per_pixel + 7)/8;
	...
	textPixmap = XCreatePixmap(xcth_display, window, 
				   disp_size.x, disp_size.y,
				   planes);
...
void DisplayDeviceX11::copyBox(int x, int y, int width, int height, 
		      int destx, int desty) {
...
void DisplayDeviceX11::postDraw() {
	...
	if(panelTextWidget) {		// bring text to panel
	    ...
	    switch(shmLevel) {
	    case shmPixmap:
		XCopyArea(xcth_display, pixmap, window, gc, 
			  0,0,					// src offet
	    ...
	    case shmImage:
	    case shmNone:
		XCopyArea(xcth_display, textPixmap, window, gc, 
			  0,0,					// src offet
			  disp_size.x, disp_size.y,		// size
			  0, 0);				// dst offset
	    }	      
	...
	case shmPixmap:
	    ...
	    break;
	case shmImage:
	    XShmPutImage(xcth_display, window, gc, image, 0,0,
			 0, 0, 
			 disp_size.x, disp_size.y, 
			 0);
	    break;
	case shmNone:
	    XPutImage(xcth_display, window, gc, image, 0,0,
		      0, 0,
		      disp_size.x, disp_size.y);
	}
    else			/* or only a part */
	...
	case shmPixmap:
	    XCopyArea(xcth_display, pixmap, window, gc, 
		      SCREEN_OFFSET_X, SCREEN_OFFSET_Y,	// src offet
		      draw_size.x, draw_size.y,		// size
		      SCREEN_OFFSET_X,			// dst offset
		      SCREEN_OFFSET_Y);
	    break;
	case shmImage:
	    XShmPutImage(xcth_display, window, gc, image,
			 ...
			 0);
	case shmNone:
	    XPutImage(xcth_display, window, gc, image, 
		      SCREEN_OFFSET_X, SCREEN_OFFSET_Y,
		      SCREEN_OFFSET_X,
		      SCREEN_OFFSET_Y,
		      draw_size.x, draw_size.y);
...
void DisplayDeviceX11::initPalette() {
	...
	case 3: draw_mode = DM_mapped3; break;
	...
	for(int i=0; i < textColors; i++) {
	    textColor[i] = 
		(red_mask   & shift(textColorRGB[i][0], red_shift  )) |
	...
	if( private_cmap == 0) {
	    ...
	    //
	    if( XAllocColorCells(xcth_display, colormap, 1, 
				 NULL, 0,	// color planes
				 pixels, 255) != 0) {
		...
		// get color cells 
		//
		if( XAllocColorCells(xcth_display, colormap, 0, 
				     NULL, 0,	// color planes
				     pixels, 128) == 0) {
		...
		for(i=0; i < 128; i++)
			...
			textPalette[i][j] = 
			    (CthughaBuffer::buffers[0].currentPalette[i*2][j] + 
			     CthughaBuffer::buffers[0].currentPalette[i*2+1][j]) >> 2;
	    } else {
		for(i=0; i < 128; i++)
		    for(j=0; j < 3; j++)
			textPalette[i][j] = 
			    (CthughaBuffer::buffers[0].currentPalette[i*2][j] + 
			     CthughaBuffer::buffers[0].currentPalette[i*2+1][j]) >> 1;
	    }		    
...
void DisplayDeviceX11::setPalette(const Palette pal) {
	...
	for(i=0; i < 256; i++) {
	    bitmap_colors0[i] = 
		(red_mask   & shift(pal[i][0], red_shift  )) |
...
void newDisplayDevice() {
    displayDevice = new DisplayDeviceX11();
}
