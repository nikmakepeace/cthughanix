...
public:
    ...
    virtual int operator()() {
        if (texture == 0) {
            ...
            gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, 32, 32, GL_RGBA, GL_UNSIGNED_BYTE, RGBA);
...
public:
    ...
    int run() {
        ...
        GLfloat offset[3] = { 0.3 * sin(now), 0, 1.4 + 0.6 * sin(now) };
        for (int i = 0; i < N; i++) {
            double alpha = now + i / double(N) * M_PI2;
            GLfloat pos[3] = { 0.1 * sin(alpha), 0.6 * sin(now * 0.2), 0.1 * cos(alpha) };

            doFly(pos[0] + offset[0], pos[1] + offset[1], pos[2] + offset[2]);
        }
...
static CoreOptionEntry* _flies[] = { new NoFly, new Rings };
