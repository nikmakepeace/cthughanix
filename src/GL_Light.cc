...
public:
    LightEntry(const char* n, const char* d)
        : CoreOptionEntry(n, d) { }
};
...
public:
    Spots1()
        : LightEntry("spots1", "RGB Spots rotating") { }
    int operator()() {
...
public:
    Spots2()
        : LightEntry("spots2", "RGB Spots pendulum") { }
    int operator()() {
        ScreenEntryGL* s = (ScreenEntryGL*)screen.current();
        if (s->type != ScreenEntryGL::object) {
            glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
...
public:
    Spots3()
        : LightEntry("with-spot", "A single white spots") { }
    int operator()() {
        ScreenEntryGL* s = (ScreenEntryGL*)screen.current();
        if (s->type != ScreenEntryGL::object) {
            glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
...
static CoreOptionEntry* _lights[] = { new NoLight, new Spots1, new Spots2, new Spots3 };
