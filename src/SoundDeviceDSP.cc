...
SoundDeviceDSP::SoundDeviceDSP(int mode) : SoundDevice(), handle(0), DMAbuffer(NULL) {
    init(mode);
}
...
int SoundDeviceDSPOut::write(const void *data, int size) {
...
SoundDeviceDSP::SoundDeviceDSP(int mode) : handle(0) {
    init(mode);
}
