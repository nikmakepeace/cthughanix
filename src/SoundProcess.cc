...
public:
    Massage1() : CoreOptionEntry("Filter1", "", 1) {}
...
public:
    Massage2() : CoreOptionEntry("Filter2", "low pass filter", 1) {}

    int operator()() {
	int temp=soundDevice->data[0][1];
	int temp2=soundDevice->data[0][0];
	for (int x=1; x < 1024; x++) {
	    soundDevice->dataProc[x][1] = soundDevice->dataProc[x-1][1] + (soundDevice->data[x][1] - temp) / 16;
	    soundDevice->dataProc[x][0] = soundDevice->dataProc[x-1][0] + (soundDevice->data[x][0] - temp2) / 16;
	    temp2=soundDevice->dataProc[x][0];
	    temp=soundDevice->dataProc[x][1];
	}
	return 0;
...
public:
    NoSoundProcess() : CoreOptionEntry("none", "", 1) {}
