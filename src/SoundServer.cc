...
void SoundServer::operator()() {
	...
	if( sendto(bcast_socket, soundDevice->data, soundDevice->rawSize, 0,
		   &(clientAddrs[i]), clientSizes[i]) == -1) {
	    if ( errno != 111)
    ...
    int acc_socket;
    if( (acc_socket = accept(request_socket, (struct sockaddr*)&my_s_addr,(socklen_t *) &size) ) >= 0) {
...
int SoundServer::remove_client(struct sockaddr my_s_addr, int /*size*/) {
	...
	for(int i=0; i < nClients; i++) {
	    ...
	    if( (ai->sin_addr.s_addr == ci->sin_addr.s_addr) &&
		(ai->sin_port == ci->sin_port) ) {

		printfv(10, "Removing entry %d\n", i);
		memcpy( &(clientAddrs[i]), &(clientAddrs[i+1]), 
			sizeof(struct sockaddr)*(nClients-i));
		memcpy( &(clientSizes[i]), &(clientSizes[i+1]), 
			sizeof(int)*(nClients-i));
		nClients --;
	    }
	}
    }
...
public:
	...
	for(int i=0; i < soundServer->nClients; i++) {
	    char str[256];
	    int addr = ntohl( 
		((struct sockaddr_in *)&soundServer->clientAddrs[i])->sin_addr.s_addr );
