...
static CoreOptionEntry * _trans[] = {
   new TranslateEntry("none", "No Translate")
};
static CoreOptionEntryList translateEntries(_trans, 1);


char lib_size[512];
static const char * translate_path[] = {
    "./",
    "./tab/",
    lib_size,
    CTH_LIBDIR "/tab/",
...
int init_translate() {
	...
	CthughaBuffer::current->translate.load(translate_path, "/tab/", ".cmd", 
					       TranslateEntry::loaderCmd);
	CthughaBuffer::current->translate.load(translate_path, "/tab/", ".tab", 
					       TranslateEntry::loaderTab);
...
// 
static int * read_trans_data(FILE * file, const tab_header & header, 
			     int BSize, const char * name) {
    union data {
	unsigned long * l;
	unsigned short * s;
    } D;
...
CoreOptionEntry * TranslateEntry::loaderTab(FILE * file, 
					    const char * name, 
					    const char * dir,
	...
	if( (header.size_x != BUFF_WIDTH) || (header.size_y != BUFF_HEIGHT)) {
	    printfv(3,"\n    Size mismatch (%dx%d instead of %dx%d)", 
		    header.size_x, header.size_y, BUFF_WIDTH, BUFF_HEIGHT);
...
CoreOptionEntry * TranslateEntry::loaderCmd(FILE * file, 
					    const char * name, 
					    const char * dir,
					    const char * total_name) {
    char line[PATH_MAX];
...
TranslateOption::TranslateOption(int buffer, const char * name) :
    ...
    lodCommonTrans(NULL), 
    lodPipe(NULL), lodPID(0), lodLine(-1), lodCurrent(-1)
{
}
